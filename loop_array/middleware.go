	...
	return func(next endpoint.Endpoint) endpoint.Endpoint {
		return func(ctx context.Context, req interface{}) (interface{}, error) {
			...
			if err != nil {
				...
				run := func(c context.Context, data interface{}, index int) (interface{}, error) {
					inner := func(index int) (interface{}, error) {
						...
						if postprocessor != nil {
							if tmp, ok := resp.(gosl.Context); ok {
				...
				if opt[RUN_WITH_OPTION.RUN_IN_TRANSACTION] {
					kit := gosl.New(ctx)
					if c, err := kit.RunInTransaction(ctx, func(ctx context.Context) (context.Context, error) {
						if arr, ok := ori.([]map[string]interface{}); ok {
							for index, item := range arr {
								resp, err := run(ctx, item, index)
								if err != nil {
									if tmp, ok := resp.(gosl.Context); ok {
										return tmp.Ctx, err
									}
